...
WHERE TemplateRowId LIKE 'test-budget-template-%'
   OR BudgetMonthRowId LIKE 'test-budget-%';
...
SELECT
    ...
    5 AS Expected
FROM BudgetMonthRow
WHERE BudgetMonthId = 'test-budget-2026-06';
...
SELECT
    ...
    5 AS Expected
FROM BudgetMonthRow
WHERE BudgetMonthId = 'test-budget-2026-05';
...
LEFT JOIN Txn t
   ...
   AND CAST(strftime('%Y', t.StartDate) AS INTEGER) = bm.Year
   AND CAST(strftime('%m', t.StartDate) AS INTEGER) = bm.Month
WHERE bmr.BudgetMonthId = 'test-budget-2026-06'
GROUP BY
    bmr.BudgetMonthRowId,
    bmr.Name,
    bmr.PlannedAmount
