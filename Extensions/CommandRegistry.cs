...
public sealed class CommandRegistry
{
    ...
    private void NormalizeOrders()
    {
        ...
        foreach (IGrouping<string, IExtMgrCommand> group in groupedCommands)
        {
            int itemOrder = 10;
            ...
            foreach (IExtMgrCommand command in group
                ...
                .ThenBy(static x => x.MenuText, StringComparer.OrdinalIgnoreCase))
            {
                if (command is ExtMgrCommandBase editable)
                {
                    editable.SetMetadata(command.MenuText, menuOrder, itemOrder);
                }
