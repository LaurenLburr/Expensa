...
public sealed class TagAssignmentControl : UserControl
{
        ...
        TagAssignmentOptions options)
    {
        ...
        _txtSearch = new TextBox
        {
            ...
            Width = 260,
            Parent = _group,
            PlaceholderText = _options.SearchPlaceholder
        };
    ...
    public void RefreshAssignedTags()
    {
        ...
        DataTable table = _db.QueryDataTable(
            ...
            BuildEntityParameters(includeTagId: false, tagId: null));
    ...
    {
        if (string.IsNullOrWhiteSpace(_entityId))
            throw new InvalidOperationException("The control must be loaded with an entity before editing tags.");

        List<DbParameter> parameters = new();

        if (!string.IsNullOrWhiteSpace(_options.EntityTypeValue))
        {
            parameters.Add(new SqliteParameter(_options.EntityTypeParameterName, _options.EntityTypeValue));
        }

        parameters.Add(new SqliteParameter(_options.EntityIdParameterName, _entityId));

        if (includeTagId)
        {
            parameters.Add(new SqliteParameter(_options.TagIdParameterName, tagId ?? string.Empty));
        }
