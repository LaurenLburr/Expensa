...
$replacementRoot = Join-Path $solutionRoot "Tools\RepositoryQueryNames"

$files = @(
    "RepositorySqlQueryNames.cs",
...
foreach ($fileName in $files) {
    ...
    if ($fileName -eq "RepositorySqlQueryNames.cs") {
    ...
    $matches = Get-ChildItem -Path $solutionRoot -Recurse -Filter $fileName |
        Where-Object {
            ...
            $_.FullName -notlike "*\Tools\RepositoryQueryNames\*"
        }
